...
vision.py           # Local image understanding via transformers moondream2 (CPU-only, opt-in, RAM-guarded >= 8GB free, lazy load once, single-analysis lock, release() on failure/shutdown)
...
- Hy-MT2 1.8B Q4_K_M uses ~1.1 GB VRAM; MiniCPM 1B F16 uses ~2 GB
- Typical usage ~3-4 GB (within 6 GB budget)
