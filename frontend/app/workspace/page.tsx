...
import { FolderOpen, Plus, Trash2, Upload, Search, Download, UploadCloud, Save, Eye, ShieldCheck, Settings2, FileText, X, Copy } from 'lucide-react';
...
export default function WorkspacePage() {
  ...
  const [chunkOverlay, setChunkOverlay] = useState<{ name: string; content: string } | null>(null);
  ...
  useEffect(() => {
    if (selectedWs) {
      ...
      setEditModel((selectedWs as unknown as Record<string, unknown>).default_model as string || '');
    }
  }, [selected, selectedWs]);
    ...
    try {
      await fetchJSON(`/v1/workspaces/${encodeURIComponent(selected)}/update`, {
        ...
        body: JSON.stringify({
          ...
          default_model: editModel.trim() || undefined,
        }),
      });
  ...
  async function searchKnowledge() {
    ...
    try {
      const data = await fetchJSON(`/v1/workspaces/${encodeURIComponent(selected)}/knowledge/search?query=${encodeURIComponent(searchQuery)}`);
  ...
  const chunks = useMemo(() => chunkOverlay ? chunkText(chunkOverlay.content) : [], [chunkOverlay]);
    ...
    <div className="page-shell space-y-6">
      ...
      {selected ? (
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_3fr] gap-6 items-start">
          {/* LEFT — Workspace configuration (40%) */}
          <div className="space-y-5">
            <Card>
              <h3 className="text-sm font-semibold mb-4 flex items-center gap-2"><Settings2 size={16} className="text-accent" /> Workspace configuration</h3>
              <div className="space-y-3">
                <div className="space-y-1.5">
                  ...
                  >
                    {workspaces.map(ws => (
                      <option key={ws.id} value={ws.id}>{ws.name} {ws.id === 'default' ? '(protected)' : ''}</option>
                    ))}
                  </select>
                </div>
                <Field label="Name">
                ...
                <Field label="Default Model (optional)">
                  <Input value={editModel} onChange={e => setEditModel(e.target.value)} placeholder="e.g. mythos-nano" />
                </Field>
            ...
            <Card>
              <h3 className="text-sm font-semibold mb-3 flex items-center gap-2"><FolderOpen size={16} className="text-accent" /> Workspaces</h3>
          ...
          <div className="space-y-5">
            <Card>
              <div className="flex items-center justify-between mb-4">
                ...
                <label className="cursor-pointer inline-flex items-center gap-2 text-sm text-accent hover:text-accent-hover px-3 py-1.5 rounded-xl border border-accent/25 bg-accent/10 transition-all hover:bg-accent/20">
                  <Upload size={16} />
                ...
                {files.map(f => (
                  <div key={f.name} className="flex items-center justify-between p-3 rounded-xl bg-bg-primary/30 border border-border hover:border-accent/30 transition-all">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button onClick={() => { openPreview(f.name); setChunkOverlay({ name: f.name, content: previewContent || '' }); }} className="p-1.5 text-text-muted hover:text-accent hover:bg-accent/10 rounded-xl transition-all" title="Chunk preview"><FileText size={14} /></button>
                      <button onClick={() => openPreview(f.name)} className="p-1.5 text-text-muted hover:text-accent hover:bg-accent/10 rounded-xl transition-all" title="Preview"><Eye size={14} /></button>
                      <button onClick={() => deleteFile(f.name)} className="p-1.5 text-text-muted hover:text-danger hover:bg-danger/10 rounded-xl transition-all"><Trash2 size={14} /></button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {chunkOverlay && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setChunkOverlay(null)}>
          <div className="glass-card w-full max-w-3xl max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-border">
              <h3 className="text-sm font-semibold flex items-center gap-2">
                <FileText size={16} className="text-accent" />
                Chunk preview: {chunkOverlay.name}
              </h3>
              <button onClick={() => setChunkOverlay(null)} className="p-1.5 rounded-lg hover:bg-bg-tertiary text-text-muted hover:text-text-primary transition-colors"><X size={16} /></button>
            </div>
            <div className="flex-1 overflow-y-auto scrollbar-thin p-4 space-y-3">
              {chunks.length === 0 && <p className="text-text-muted text-sm">Empty file</p>}
              {chunks.map((chunk, i) => (
                <div key={i} className="rounded-xl border border-border bg-bg-primary/40 p-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[10px] font-mono text-text-muted">Chunk {i + 1} · ~{chunk.length} chars</span>
                    <button onClick={() => { navigator.clipboard.writeText(chunk); addToast('Chunk copied', 'success'); }} className="p-1 rounded hover:bg-bg-tertiary text-text-muted hover:text-accent transition-colors" title="Copy chunk"><Copy size={12} /></button>
                  </div>
                  <pre className="text-xs text-text-secondary whitespace-pre-wrap leading-relaxed">{chunk}</pre>
                </div>
              ))}
            </div>
            <div className="p-3 border-t border-border text-xs text-text-muted flex items-center justify-between">
              <span>{chunks.length} chunks · {CHUNK_SIZE} chars each · {CHUNK_OVERLAP} overlap</span>
              <Button variant="secondary" size="sm" onClick={() => setChunkOverlay(null)}>Close</Button>
            </div>
          </div>
        </div>
      )}
